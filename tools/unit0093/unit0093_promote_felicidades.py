...
CAND = ROOT / "imports/unit0093_felicidades/UNIT0093_CHEO_FELICIDADES_TITLE_CANDIDATES.tsv"
...
BACKUP_DIR = ROOT / "authority/backups/unit0093"
STAGING_DIR = ROOT / "authority/staging/unit0093"
REPORT_DIR = ROOT / "reports/unit0093"
...
for p in [REC_MASTER, SONG_MASTER, PERF_MASTER, RELEASE_MASTER, LABEL_MASTER]:
    (BACKUP_DIR / f"{p.name}.before_unit0093.tsv").write_text(p.read_text(encoding="utf-8"), encoding="utf-8")
...
write_tsv(STAGING_DIR / "UNIT0093_SONGS_ADDED.tsv", song_fields, songs_added)
write_tsv(STAGING_DIR / "UNIT0093_RECORDINGS_ADDED.tsv", rec_fields, recs_added)

with (REPORT_DIR / "UNIT0090_PROMOTION_REPORT.md").open("w", encoding="utf-8") as fh:
    fh.write("# UNIT-0093 Promotion Report\n\n")
    fh.write("Project: MUSIC_RELATIONSHIPS_DB\n")
    fh.write("Unit: UNIT-0093\n")
    fh.write("Target: Cheo Feliciano / Felicidades relationships\n\n")
    fh.write("## Totals\n\n")
    ...
    for r in recs_added:
        fh.write(f"- {r['recording_id']} — {r['canonical_recording_title']} ↔ Cheo Feliciano ↔ Felicidades ↔ Fania ↔ SRC0003\n")
