...
CAND = ROOT / "imports/unit0092_sentimiento_tu/UNIT0092_CHEO_SENTIMIENTO_TU_TITLE_CANDIDATES.tsv"
...
BACKUP_DIR = ROOT / "authority/backups/unit0092"
STAGING_DIR = ROOT / "authority/staging/unit0092"
REPORT_DIR = ROOT / "reports/unit0092"
...
for p in [REC_MASTER, SONG_MASTER, PERF_MASTER, RELEASE_MASTER, LABEL_MASTER]:
    (BACKUP_DIR / f"{p.name}.before_unit0092.tsv").write_text(p.read_text(encoding="utf-8"), encoding="utf-8")
...
write_tsv(STAGING_DIR / "UNIT0092_SONGS_ADDED.tsv", song_fields, songs_added)
write_tsv(STAGING_DIR / "UNIT0092_RECORDINGS_ADDED.tsv", rec_fields, recs_added)

with (REPORT_DIR / "UNIT0090_PROMOTION_REPORT.md").open("w", encoding="utf-8") as fh:
    fh.write("# UNIT-0092 Promotion Report\n\n")
    fh.write("Project: MUSIC_RELATIONSHIPS_DB\n")
    fh.write("Unit: UNIT-0092\n")
    fh.write("Target: Cheo Feliciano / Sentimiento Tú relationships\n\n")
    fh.write("## Totals\n\n")
    ...
    for r in recs_added:
        fh.write(f"- {r['recording_id']} — {r['canonical_recording_title']} ↔ Cheo Feliciano ↔ Sentimiento Tú ↔ Fania ↔ SRC0003\n")
